 ...
 */
const isFuzzyMatch = (input, target, threshold = GAME_CONFIG.FUZZY_MATCH_THRESHOLD) => {
  return levenshteinDistance(normalizeText(input), normalizeText(target)) <= threshold;
};
...
const checkAnswer = (userAnswer, correctWord, correctMeaning, exactMode = false) => {
  ...
  if (!exactMode) {
    ...
    if (normalizedMeanings.some(meaning => isFuzzyMatch(normalizedAnswer, meaning))) return true;
  }
...
const LexiFlashScreen = ({ category }) => {
  ...
  const totalCards = useMemo(() => deck.length + correctCards.length, [deck.length, correctCards.length]);
  ...
  useEffect(() => {
    ...
    const initializeGame = () => {
      const filteredCards = cardExamples.filter((card) => card.deckId === category.id);
  ...
  useEffect(() => {
    if (deck.length > 0 && !currentCard && phase === GAME_PHASES.IDLE && initialCardsLoaded) {
      startNewCard();
    }
  ...
  const startCountdown = () => {
    ...
    const totalSteps = (GAME_CONFIG.COUNTDOWN_DURATION * 1000) / GAME_CONFIG.COUNTDOWN_INTERVAL;
    let currentStep = 0;
    
    countdownIntervalRef.current = setInterval(() => {
      currentStep++;
      count = Math.ceil(GAME_CONFIG.COUNTDOWN_DURATION - (currentStep * GAME_CONFIG.COUNTDOWN_INTERVAL) / 1000);
  ...
  const startNewCard = async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, GAME_CONFIG.AUDIO_DELAY));
  ...
  const handleSubmit = (event) => {
    ...
    // Validar fase actual
    if (phase !== GAME_PHASES.REFLECTION && phase !== GAME_PHASES.ANSWER) return;
    ...
    <div className="space-y-6">
      <div
        className={`p-8 rounded-xl border-2 transition-all ${
          isListening ? "border-red-400 bg-red-50" : "border-slate-300 bg-slate-50"
        }`}
      >
        {userInput ? (
      ...
      case GAME_PHASES.ANSWER:
        return (
          <div className="space-y-8">
            ...
            {inputMode === INPUT_MODES.TEXT ? renderTextInput() : renderVoiceInput()}
  ...
  const renderCardHeader = () => {
    let phaseText = "";
    switch (phase) {
      case GAME_PHASES.LISTENING_WORD: phaseText = "Escuchando"; break;
      case GAME_PHASES.LISTENING_CONTEXT: phaseText = "Contexto"; break;
      case GAME_PHASES.REFLECTION: phaseText = "Reflexión"; break;
      default: phaseText = "Tu respuesta";
    }
    ...
    return (
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${
              phase === GAME_PHASES.LISTENING_WORD || phase === GAME_PHASES.LISTENING_CONTEXT
        ...
        <button
          ...
          title={!audioAvailable ? "Disponible después del contexto" : "Repetir audio"}
        >
          <Volume2 size={18} />
        ...
        <div className={`p-6 ${
          ...
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-800 mb-2">
              {feedback === FEEDBACK_TYPES.CORRECT ? "¡Correcto!" : "Respuesta Incorrecta"}
            </h2>
            <p className="text-slate-600">
              {timeoutExpired 
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 font-sans">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {isLoading ? (
          renderLoadingScreen()
        ) : deck.length === 0 && correctCards.length === 0 && !isLoading && initialCardsLoaded ? (
